...
```{r}
ny_noaa %>% 
  summarize(median = median(snow_in, na.rm = TRUE))
```
...
The plot shows that the max temperature is higher in July than January across all years and there is variation from year to year. Adding the geom_smooth function shows that max temperature has increased since 1980 in January. There were a few observable outliers. For instance, the low of -5 degrees C in Januart of 1994.

I will make a two-panel plot showing the maximum vs minimum temperature for the full dataset. 

```{r}
ny_noaa %>% 
  select(year, tmax, tmin) %>% 
  drop_na() %>% 
  group_by(year, tmax, tmin) %>% 
  ggplot(aes(x = year, y = tmax)) +
  labs(
    title = "Max and min temperature by year",
    x = "Year",
    y = "Maxiumum daily temperature (C)",
    caption = "Data from the rnoaa package"
  ) 
...
Finally, I'll plot the distribution of snowfall above 0 mm and below 100mm separately by year.  
```{r}
ny_noaa %>% 
  select(year, snow) %>% 
  drop_na() %>% 
